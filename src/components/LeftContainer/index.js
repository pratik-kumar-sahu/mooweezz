...
function LeftContainer({ movies, selectedMovie, setSelectedMovie }) {
  ...
  const RECOMMENDS = `https://api.themoviedb.org/3/movie/${selectedMovie.id}/recommendations?api_key=${API_KEY}&language=en-US&page=1`;
  ...
  return (
    <div id="left-container" className="left-container">
      <div className='hide-it' ref={dummy}></div>
      {selectedMovie && recommends ? (
        <div className="left-container__recommends">
          <h3>RECOMM...</h3>
          <div>
            {recommends &&
          ...
          <div className="left-container__main">
            ...
            <div className="left-container__main--extra">
              <div className="left-container__main--extra-add">
                ...
                >
                  {favourites.filter((e) => e.data.id === selectedMovie.id)
                    .length
                    ? "Remove ❤️"
                    : "Add ❤️"}
                </button>
